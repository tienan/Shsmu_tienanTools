...
nrow(clinical_LUAD)
######################
clinical_LUAD_DL = merge(DL_statu,clinical_LUAD,by.x = "name",by.y="submitter_id")
clinical_LUAD_DL$group = ifelse(as.numeric(clinical_LUAD_DL$gene_name_exp_carcer_sign_sum)>10,1,0)
TCGAanalyze_survival(clinical_LUAD_DL,
                     c("group",
...
require("survival")
library(survminer)
sink(file = "TCGA_DL.txt",append = T)
print(CancerName)
